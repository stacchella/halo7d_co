...
def build_obs(objid=1, data_table=path_wdir + 'data/halo7d_with_phot.fits', err_floor=0.05, S2N_cut=1.0, remove_mips24=False, **kwargs):
    ...
    if (SN_calc < S2N_cut):
        print 'S/N =', SN_calc
        print 'do not fit this galaxy bc SN...'
        obs['filters'] = None
    ...
    if (catalog[idx_cat]['exclusion_flag'] == 1.0):
        print 'do not fit this galaxy bc exclusion flag...'
        obs['filters'] = None
        obs['wave_effective'] = None
        obs['maggies'] = None
        obs['maggies_unc'] = None
