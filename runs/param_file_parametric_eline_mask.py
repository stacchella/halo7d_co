...
def build_obs(objid=1, data_table=path_wdir + 'data/halo7d_with_phot.fits', err_floor_phot=0.05, err_floor_spec=0.1, S2N_cut=1.0, remove_mips24=False, switch_off_phot=False, switch_off_spec=False, **kwargs):
    ...
    for ii in len(line_wave):
        if (ii == 0):
            ...
            EL_region = EL_region | ((catalog[idx_cat]['LAM'].data > (catalog[idx_cat]['ZSPEC']+1.0)*line_wave[ii]-dA) & (catalog[idx_cat]['LAM'].data < (catalog[idx_cat]['ZSPEC']+1.0)*line_wave[ii]+dA))
    obs['mask'] = obs['mask'] & ~EL_region
    return obs
